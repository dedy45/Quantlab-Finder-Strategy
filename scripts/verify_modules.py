"""
QUANT LAB - MODULE VERIFICATION
===============================
Verifikasi semua modul FASE 0, 1 & 2 berfungsi dengan benar.
...
print("=" * 70)
print("QUANT LAB - MODULE VERIFICATION")
print("=" * 70)
...
if cache_path.exists():
    ...
    log_info("Generating synthetic data for verification...")
    np.random.seed(42)
...
# STEP 6: STRATEGIES (FASE 2)
print("=" * 70)
print("STEP 6: STRATEGIES (FASE 2)")
print("=" * 70)

print("\n--- Momentum Strategy ---")
try:
    from strategies import MomentumStrategy, MomentumConfig
    config = MomentumConfig(fast_period=10, slow_period=30)
    strategy = MomentumStrategy(config)
    strategy.fit(prices_df[['F_GC']].rename(columns={'F_GC': 'close'}))
    signals = strategy.predict(prices_df[['F_GC']].rename(columns={'F_GC': 'close'}))
    log_info(f"Signals: {signals.value_counts().to_dict()}")
    log_pass("Momentum Strategy VERIFIED")
except Exception as e:
    log_fail(f"Momentum error: {e}")

print("\n--- Mean Reversion Strategy ---")
try:
    from strategies import MeanReversionStrategy, MeanReversionConfig
    config = MeanReversionConfig(lookback=20, entry_z=2.0)
    strategy = MeanReversionStrategy(config)
    signals = strategy.fit_predict(prices_df[['F_GC']].rename(columns={'F_GC': 'close'}))
    log_info(f"Signals: {signals.value_counts().to_dict()}")
    log_pass("Mean Reversion Strategy VERIFIED")
except Exception as e:
    log_fail(f"Mean Reversion error: {e}")

print("\n--- ML Strategy ---")
try:
    from strategies import MLStrategy, MLStrategyConfig
    config = MLStrategyConfig(n_estimators=50, max_depth=3)
    strategy = MLStrategy(config)
    signals = strategy.fit_predict(prices_df[['F_GC']].rename(columns={'F_GC': 'close'}))
    log_info(f"Signals: {signals.value_counts().to_dict()}")
    log_pass("ML Strategy VERIFIED")
except Exception as e:
    log_fail(f"ML Strategy error: {e}")

print()

# STEP 7: VALIDATION PIPELINE (FASE 2)
print("=" * 70)
print("STEP 7: VALIDATION PIPELINE (FASE 2)")
print("=" * 70)

print("\n--- Walk-Forward Validation ---")
try:
    from core.validation_engine import WalkForwardValidator
    from strategies import MomentumStrategy, MomentumConfig
    strategy = MomentumStrategy(MomentumConfig(fast_period=10, slow_period=30))
    validator = WalkForwardValidator(n_splits=3, purge_gap=5)
    result = validator.validate(strategy, prices_df[['F_GC']].rename(columns={'F_GC': 'close'}))
    log_info(f"Splits: {result.n_splits}, Degradation: {result.avg_sharpe_degradation:.2%}")
    log_pass("Walk-Forward Validation VERIFIED")
except Exception as e:
    log_fail(f"Walk-Forward error: {e}")

print("\n--- Purged K-Fold CV ---")
try:
    from core.validation_engine import PurgedKFold
    cv = PurgedKFold(n_splits=5, purge_pct=0.01)
    splits = list(cv.split(prices_df.values))
    log_info(f"Generated {len(splits)} purged splits")
    log_pass("Purged K-Fold VERIFIED")
except Exception as e:
    log_fail(f"Purged K-Fold error: {e}")

print("\n--- Robustness Testing ---")
try:
    from core.validation_engine import RobustnessTester
    from strategies import MomentumStrategy, MomentumConfig
    strategy = MomentumStrategy(MomentumConfig(fast_period=10, slow_period=30))
    tester = RobustnessTester(n_simulations=20)
    result = tester.test(strategy, prices_df[['F_GC']].rename(columns={'F_GC': 'close'}))
    log_info(f"Tests: {result.n_tests}, Pass Rate: {result.pass_rate:.1%}")
    log_pass("Robustness Testing VERIFIED")
except Exception as e:
    log_fail(f"Robustness error: {e}")
...
if len(results['failed']) == 0:
    ...
    print("\n" + "=" * 70)
    print("  VERIFICATION INCOMPLETE - CHECK ERRORS ABOVE")
    print("=" * 70)
    sys.exit(1)
